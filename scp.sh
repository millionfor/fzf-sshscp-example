...
_scp_args() {
    index=$(_index "$1")
    local sshcmd="${cmds[index]}"
    # 去掉 ssh 前缀，转换 -p 为 scp 的 -P，其它 -i/-o 直接复用
    local args="${sshcmd#ssh}"
    args="${args# }"
    # 开头是 -p 的情况
    if [[ "$args" == -p\ * ]]; then
        args="-P ${args#-p }"
...
_preview2() {
    index=$(_index "$1")
    target="$2"
    [ "$target" = "从本地到远程" ] && echo "scp -r $(_scp_args \"$1\") [SOURCE] ${targets[index]}:~"
    [ "$target" = "从远程到本地" ] && echo "scp -r $(_scp_args \"$1\") ${targets[index]}:[SOURCE] ."
}
...
_scp() {
    ...
    [ "$target" = "从远程到本地" ] && _source=$(eval "$( _ssh_cmd \"$server\" ) ls" | fzf $fzfopt --print-query --header=请选择要下载的文件 --height=10 | sed '/^$/d' | sed -n '$p')
    [[ -z "$_source" ]] && exit 0

    # 组装命令
    [ "$target" = "从本地到远程" ] && _cmd="scp -r $(_scp_args \"$server\") ${_source/'~'/$HOME} ${targets[index]}:~"
    [ "$target" = "从远程到本地" ] && _cmd="scp -r $(_scp_args \"$server\") ${targets[index]}:$_source ."
